...
{
#ifdef CONFIG_MACH_KTTECH
	void *restart_reason_bootmem;
	restart_reason_bootmem = ioremap_nocache(MSM_BOOTMEM_RESTART_MAGIC_ADDR, SZ_4K);	
	writel(0x6f656d10, restart_reason_bootmem);
	dsb(); // Cache Flush Before io unmap
	iounmap(restart_reason_bootmem);
	__raw_writel(0x77665505, restart_reason);//Normal Reboot
	printk("Finished restart_reason write \n");
#endif
...
#ifdef CONFIG_MACH_KTTECH //OEM-KTTECH (FTM mode)
...
#ifdef CONFIG_MACH_KTTECH //DEFAULT REASON (ASSUME: RESET in caseo of no comamnd)
  else
    {
        if(kttech_in_panic == 1)
        {
     		__raw_writel(0x77665503, restart_reason);//Panic Reboot
           kttech_in_panic =0;
        }else{
        	__raw_writel(0x77665505, restart_reason);//Normal Reboot
        }
...
#ifdef CONFIG_MACH_KTTECH
	__raw_writel(0x77665504, restart_reason);//initial restart_reason
#endif
