...
static long unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
             ...
             case TSPDRV_TUNING_ARG1: //M
                    printk("[tspdrv] TSPDRV_TUNING_ARG1 arg : %lu\n", arg);
                    g_nLRA_PWM_M = arg;
                    break;
             case TSPDRV_TUNING_ARG2: //N & D
                    printk("[tspdrv] TSPDRV_TUNING_ARG2 arg : %lu\n", arg);
                    g_nLRA_PWM_N = arg;
                    g_nLRA_PWM_D= arg >> 1;
                    break;
             case TSPDRV_TUNING_ARG3: //Multiplier
                    printk("[tspdrv] TSPDRV_TUNING_ARG2 arg : %lu\n", arg);
                    g_nLRA_PWM_Multiplier = arg;
                    break;
#endif					
#if defined(CONFIG_KTTECH_MODEL_O3) || defined(CONFIG_KTTECH_MODEL_O4)
