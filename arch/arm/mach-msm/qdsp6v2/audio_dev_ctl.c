...
{
	pr_debug("%s: voice call = %d\n", __func__, routing_info.isvoicecall);
	return routing_info.isvoicecall;
}
EXPORT_SYMBOL(msm_device_get_isvoice);
...
#ifdef CONFIG_KTTECH_SOUND // 20110207 by ssgun - check voice call
	if(evt_id == AUDDEV_EVT_START_VOICE) {
		pr_debug("%s: AUDDEV_EVT_START_VOICE\n", __func__);
	} else if(evt_id == AUDDEV_EVT_END_VOICE) {
		routing_info.isvoicecall = 0;
		pr_debug("%s: AUDDEV_EVT_END_VOICE\n", __func__);
	}
#endif /*CONFIG_KTTECH_SOUND*/
