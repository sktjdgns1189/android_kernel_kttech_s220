...
#if defined(CONFIG_KTTECH_SOUND_WM8993)
	trc = clk_enable(drv->rx_osrclk);
	if (trc != 0) {
		pr_err("%s Unable to enable i2s_spkr_osr_clk\n", __func__);
		goto error_invalid_freq;
	}
#else
...
#if defined(CONFIG_KTTECH_SOUND_WM8993)
	trc = clk_enable(drv->rx_bitclk);
	if (trc != 0) {
		pr_err("%s Unable to enable i2s_spkr_bit_clk\n", __func__);
		goto error_adie;
	}
#else
...
#if defined(CONFIG_KTTECH_SOUND_WM8993)
	trc = clk_enable(drv->tx_osrclk);
	if (trc != 0) {
		pr_err("%s Unable to enable i2s_mic_osr_clk\n", __func__);
		goto error_invalid_freq;
	}
#else
...
#if defined(CONFIG_KTTECH_SOUND_WM8993)
	if (IS_ERR_VALUE(trc)) {
		pr_err("%s: ERROR setting m clock1\n", __func__);
		goto error_adie;
	}
#endif

#if defined(CONFIG_KTTECH_SOUND_WM8993)
	trc = clk_enable(drv->tx_bitclk);
	if (trc != 0) {
		pr_err("%s Unable to enable i2s_mic_bit_clk\n", __func__);
		goto error_adie;
	}
#else
...
module_param(msm_codec_i2s_slave_mode, bool, 1);
#else
...
#ifdef CONFIG_KTTECH_I2S_MSM_SLAVE // 20110210 by ssgun - slave mode
	msm_codec_i2s_slave_mode = 1; // 20110210 by ssgun : 0 -> 1
