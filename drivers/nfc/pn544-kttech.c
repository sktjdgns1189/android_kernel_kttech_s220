...
// modify : kkong120203, kkong120322
// description : 
// S=====================================================================
typedef enum{
...
#define _PN544_I2C_TEST  // htdo_0511 

#define MAX_BUFFER_SIZE	512
#ifdef _PN544_I2C_LOG_VIEW
#define MAX_PRINTABLE_BUFFER_SIZE	64
static bool log_enable = false;  //htdo_0511
#endif
		...
		size_t count, loff_t *offset)
{
    ...
    if(log_enable)
    {
        ...
        printk(KERN_INFO "nfc_check..  %s Data: (%zu bytes)\n", __func__, count);
        ...
        for(i=0; i<count; i++)
        {
            ...
            if(i%20 == 0 && i !=0)
            {
                printk(KERN_INFO "nfc_check..  %s", print_tmp);
                memset((char*)print_tmp, 0, sizeof(print_tmp));
        ...
        printk(KERN_INFO "nfc_check..  ven:%d(%x), irq:%d(%x), firm:%d(%x)", pn544_dev->ven_gpio, gpio_get_value(pn544_dev->ven_gpio), 
    ...
    if (ret < 0) {
        printk(KERN_ERR "nfc_check..  %s: fail!!!!!!i2c_master_recv returned %d\n", __func__, ret);
        return ret;
    }
    if (ret > count) {
        printk(KERN_ERR "nfc_check..  %s: fail!!!!!!received too many bytes from i2c (%d)\n", __func__, ret);
        return -EIO;
    }
    if (copy_to_user(buf, tmp, ret)) {
        printk(KERN_ERR "nfc_check..  %s : failed to copy to user space\n", __func__);
        return -EFAULT;
    }
    return ret;
		...
		size_t count, loff_t *offset)
{
    ...
    if (copy_from_user(tmp, buf, count)) {
        printk(KERN_INFO "nfc_check..  %s : failed to copy from user space\n", __func__);
        return -EFAULT;
    }
...
#ifdef _PN544_I2C_LOG_VIEW		
    if(log_enable)
        printk(KERN_ERR "nfc_check..  %s error!!!!!!!!(%d)\n", __func__, ret);
#endif		
        ret = -EIO;
    }
    return ret;
...
static int pn544_dev_test(struct i2c_client *client)
{
    ...
    if (ret < 0) {
        printk(KERN_ERR "nfc_check..  %s : i2c_master_send returned %d\n", __func__, ret);
        ret = -EIO;
    }
    ...
    if (ret < 0) {
        printk(KERN_ERR "nfc_check..  %s : i2c_master_recv returned %d\n", __func__, ret);
        ret = -EIO;
    }

    printk(KERN_ERR "nfc_check..  %s : get data %d %d %d %d\n", __func__, tmpRx[0], tmpRx[1], tmpRx[2], tmpRx[3]);
...
static int pn544_dev_open(struct inode *inode, struct file *filp)
{
    ...
    if(log_enable)
    {
        printk(KERN_INFO "nfc_check..  ven:%d(%x), irq:%d(%x), firm:%d(%x)", g_pn544_dev->ven_gpio, gpio_get_value(g_pn544_dev->ven_gpio), 
    ...
    printk(KERN_INFO "nfc_check..  %s : %d,%d\n", __func__, imajor(inode), iminor(inode));
...
static int pn544_kttech_cmd_process(unsigned long command)
{
    ...
    if(log_enable) {
        printk(KERN_INFO "nfc_check..  %s:  command(%ld)\n", __func__, command);
    }
    ...
    switch(command)
    {
        ...
        case KTTECH_CMD_SET_CLK_REQ_PIN_MODE:
            ...
            if (ret < 0) {
                printk(KERN_ERR "nfc_check..  %s:  Failed to vote for TCX0_D1 ON (%d)\n", __func__, ret);
            }
            break;
#endif

        default:
            printk(KERN_ERR "nfc_check..  %s:  Invalid Parameter (%ld)\n", __func__, command);
            break;
    }
...
#endif
//--sungwoo_bae@mo2.co.kr--0504 ifdef~endif
// E=====================================================================
    ...
    switch (cmd) {
        case PN544_SET_PWR:
            if (arg == 2) {
                ...
                */
                printk(KERN_INFO "nfc_check..  %s power on with firmware\n", __func__);
                gpio_set_value(pn544_dev->ven_gpio, 1);
                ...
                /* power on */
                printk(KERN_INFO "nfc_check..  %s power on, gpio:[%d] [%d]\n", __func__, pn544_dev->firm_gpio, pn544_dev->ven_gpio);
                gpio_set_value(pn544_dev->firm_gpio, 0);
                gpio_set_value(pn544_dev->ven_gpio, 1);
                msleep(20);
            } else  if (arg == 0) {
                /* power off */
                printk(KERN_INFO "nfc_check..  %s power off\n", __func__);
                gpio_set_value(pn544_dev->firm_gpio, 0);
                gpio_set_value(pn544_dev->ven_gpio, 0);
                msleep(60);
#ifdef READ_IRQ_MODIFY
                ...
                printk(KERN_INFO "nfc_check..  %s bad arg\n", __func__);
                return -EINVAL;
            }
            break;
        ...
        default:
            printk(KERN_INFO "nfc_check..  %s bad ioctl %d\n", __func__, cmd);
            return -EINVAL;
    }
		...
		const struct i2c_device_id *id)
{
    ...
    if (platform_data == NULL) {
        printk(KERN_INFO "nfc_check..  %s : nfc probe fail\n", __func__);
        return  -ENODEV;
    }

#ifdef _PN544_I2C_LOG_VIEW
    if(log_enable)
        printk(KERN_INFO "nfc_check..  %s : nfc probe start, firm_gpio:%d, ven_gpio:%d\n", __func__, gpio_get_value(platform_data->firm_gpio), gpio_get_value(platform_data->ven_gpio));
#endif

    #ifdef NFC_USE_MSM_XO
    nfc_clock = msm_xo_get(MSM_XO_TCXO_D0, "nfc_clock");
    if (IS_ERR(nfc_clock)) {
		ret = PTR_ERR(nfc_clock);
		printk(KERN_ERR "nfc_check..  %s: Couldn't get TCXO_D0 vote for NFC (%d)\n",
					__func__, ret);
	}

	ret = msm_xo_mode_vote(nfc_clock, MSM_XO_MODE_PIN_CTRL);
	if (ret < 0) {
		printk(KERN_ERR "nfc_check..  %s:  Failed to vote for TCX0_D0 ON (%d)\n",
					__func__, ret);
	}
	#endif
    ...
    printk(KERN_INFO "nfc_check..  nfc probe platform data (irq%d:%d) (ven%d:%d) (firm%d:%d)\n",
    ...
    if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
        printk(KERN_INFO "nfc_check..  %s : need I2C_FUNC_I2C\n", __func__);
        return  -ENODEV;
    }
    ...
    if (ret) {
        printk(KERN_INFO "nfc_check..  %s : misc_register failed\n", __FILE__);
    ...
    */
    printk(KERN_INFO "nfc_check..  %s : requesting IRQ %d\n", __func__, client->irq);
    pn544_dev->irq_enabled = true;
...
static int __init pn544_dev_init(void)
{
    ...
    printk(KERN_INFO "nfc_check..  Loading pn544 driver\n");
    ret = i2c_add_driver(&pn544_driver);
    printk(KERN_INFO"nfc_check..  %s: i2c_add_driver result %d\n", __func__, ret);
...
{
    printk(KERN_INFO "nfc_check..  Unloading pn544 driver\n");
