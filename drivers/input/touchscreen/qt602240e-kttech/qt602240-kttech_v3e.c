/*
*  qt602240-kttech_v3e.c - Atmel maXTouch Touchscreen Controller
*
*  Version 0.2a
*
*  An early alpha version of the maXTouch Linux driver.
*
*  Copyright (C) 2011 JhoonKim <jhoonkim@kttech.co.kr>
*
...
#include "qt602240-kttech_v3e.h"
#include "qt602240-kttech_v3e_cfg.h"
#include "qt602240-kttech_v3e_config.h"

int qt602240_get_proximity_enable_state = 0;
...
#if 0
extern struct class *kttech_class;
...
#ifdef CONFIG_TOUCH_DEBUG_KTTECH
		if (debug >= DEBUG_MESSAGES)
...
#if 0	/* 11.11.01. Debug. Next charger tuning apply. */
		/* CHRGON enable */
...
#ifdef KEY_LED_CONTROL
static void key_led_off_dwork(struct work_struct *work)
{
	int rc = 0;

	if (debug >= DEBUG_MESSAGES) 
		pr_info("[TSP_KEY] TSP Key LED Off.\n");

#ifdef TSP_KEY_LED_ENABLE
	rc += pm8058_set_led_current(PMIC8058_ID_LED_2, 0);
	msleep(1);
#endif
#ifdef TSP_HOME_LED_ENABLE
	rc += pm8058_set_led_current(PMIC8058_ID_LED_KB_LIGHT, 0);
	msleep(1);
#endif

	if(rc){
		pr_err("[TSP] pm8058_set_led_current failed.");
	}
	else {
...
static void key_led_on(struct mxt_data *mxt)
{
	int rc = 0;

	if (debug >= DEBUG_MESSAGES) 
		pr_info("[TSP_KEY] TSP Key LED On.\n");

	if(delayed_work_pending(&mxt->led_off_dwork))
		cancel_delayed_work(&mxt->led_off_dwork);

	if(key_led_status == false) {
#ifdef TSP_KEY_LED_ENABLE
		rc += pm8058_set_led_current(PMIC8058_ID_LED_2, KEY_LED_DEFAULT_BRIGHTNESS);
		msleep(1);
#endif
#ifdef TSP_HOME_LED_ENABLE
		rc += pm8058_set_led_current(PMIC8058_ID_LED_KB_LIGHT, KEY_LED_DEFAULT_BRIGHTNESS);
		msleep(1);
#endif
...
static irqreturn_t home_key_irq(int irq, void *_mxt) {

	struct	mxt_data *mxt = _mxt;
	int home_key_status;

	home_key_status = gpio_get_value(KEY_IRQ_HOME);
	wmb();

	printk("[TSP] Home Key Status : %d\n", home_key_status);
	
	if(home_key_status) {	/* Home Key Released */
		input_report_key(mxt->input, KEY_HOME, KEY_RELEASE);
#ifndef ENABLE_FATORY_TUNING
		/* Start Calibration */
		if(cal_check_flag != 1u)
			calibrate_chip(mxt);
#endif
		/* Release All Keys */
		mxt_release_all_keys(mxt);
#ifdef KEY_LED_CONTROL		
		schedule_delayed_work(&mxt->led_off_dwork, KEY_LED_TIME_OFF_DELAY);
#endif
	}
...
void Process_Touch_KEY(struct mxt_data *mxt, struct multi_touch_info *mtouch_info, int i){
	u8 status = 0;
	...
	if (prev_touch_id >= touch_id || pressed_or_released) { 
		for (i = 0; i < MXT_MAX_NUM_TOUCHES; ++i) { 
			...
			if((TOUCH_3KEY_AREA_Y_BOTTOM > mtouch_info[i].y) && 
				...
				Process_Touch_KEY(mxt, mtouch_info, i);
			}
			else {
				REPORT_MT(i, mtouch_info[i].x, mtouch_info[i].y, mtouch_info[i].pressure, mtouch_info[i].size);
...
static int calibrate_chip(struct mxt_data *mxt)
{
	...
	ret = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_TOUCH_MULTITOUCHSCREEN_T9) + MXT_ADR_T9_TCHTHR, T9_TCHTHR_CAL);
...
static void check_chip_palm(struct mxt_data *mxt)
{
	...
	if((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00)) {
		...
		/* For Pre-defined Max Touch Channels
		max_touch_ch = CAL_CFG_MAX_TOUCH_CH;
		*/
...
static void check_chip_calibration(struct mxt_data *mxt)
{
	...
	/* For Pre-defined Calibration Configurations
	CAL_THR = CAL_CFG_CAL_THR;
	num_of_antitouch = CAL_CFG_MAX_ATCH_NUM;
	*/
...
static void cal_maybe_good(struct mxt_data *mxt)
{
	...
	if (mxt_time_point != 0) {
		...
		if ((jiffies_to_msecs(jiffies) - mxt_time_point) >= 300) {
			...
			ret = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_TOUCH_MULTITOUCHSCREEN_T9) + MXT_ADR_T9_NUMTOUCH, T9_NUMTOUCH);
			//ret = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_PROCI_TOUCHSUPPRESSION_T42) + MXT_ADR_T42_MAXNUMTCHS, T42_MAXNUMTCHS);
...
{ 
	...
	msleep(50);  /*typical value is 150ms*/
	calibrate_chip(mxt);
...
param_check_ok:
...
#ifdef KEY_LED_CONTROL
	INIT_DELAYED_WORK(&mxt->led_off_dwork, key_led_off_dwork);
#endif
	...
	* been sent before interrupts were enabled. */
	...
	/* O6 Home Key Handler */
	gpio_tlmm_config(GPIO_CFG(KEY_IRQ_HOME, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_4MA), GPIO_CFG_ENABLE);
	
	error = request_threaded_irq(MSM_GPIO_TO_INT(KEY_IRQ_HOME),
		NULL,
		home_key_irq,
	...
	error = enable_irq_wake(MSM_GPIO_TO_INT(KEY_IRQ_HOME));
	...
	if (debug > DEBUG_INFO)
		dev_info(&client->dev, "[TSP] touchscreen, key %d\n", KEY_IRQ_HOME);
