...
/* Touch Booster */
#ifdef CONFIG_KTTECH_DVFS
#define KTTECH_TOUCH_BOOSTER    1
#define TOUCH_BOOSTER_OFF_TIME  100
#define TOUCH_BOOSTER_CHG_TIME  200
#else
#define KTTECH_TOUCH_BOOSTER    0
#endif
...
/* Touch Booster */
#if KTTECH_TOUCH_BOOSTER
static void change_dvfs_lock(struct work_struct *work)
{
	struct mxt_data *data = container_of(work, struct mxt_data, work_dvfs_chg.work);
	int ret = 0;

	mutex_lock(&data->dvfs_lock);
	ret = cpufreq_set_limit(TOUCH_BOOSTER_1ST_STOP, 0);
...
{
	struct mxt_data *data = container_of(work, struct mxt_data, work_dvfs_off.work);

	mutex_lock(&data->dvfs_lock);

	cpufreq_set_limit(TOUCH_BOOSTER_1ST_STOP, 0);
	cpufreq_set_limit(TOUCH_BOOSTER_2ND_STOP, 0);
...
static void set_dvfs_lock(struct mxt_data *data, uint32_t on)
{
	...
	if (on == MXT_TOUCH_BOOST_OFF) {
		if (data->dvfs_lock_status) {
			cancel_delayed_work(&data->work_dvfs_chg);
			schedule_delayed_work(&data->work_dvfs_off,
				msecs_to_jiffies(TOUCH_BOOSTER_OFF_TIME));
		}
	} else if (on == MXT_TOUCH_BOOST_ON) {
			cancel_delayed_work(&data->work_dvfs_off);

			if (!data->dvfs_lock_status) {
				ret += cpufreq_set_limit(TOUCH_BOOSTER_2ND_START, 0);
				ret += cpufreq_set_limit(TOUCH_BOOSTER_1ST_START, 0);
				...
				if (debug >= DEBUG_VERBOSE)
					pr_info("[TSP] DVFS On![%d]", on);
			}
	} else if (on == MXT_TOUCH_BOOST_RESET) {
			cancel_delayed_work(&data->work_dvfs_off);
			cancel_delayed_work(&data->work_dvfs_chg);
...
#if KTTECH_TOUCH_BOOSTER
	set_dvfs_lock(mxt, MXT_TOUCH_BOOST_RESET);
...
#if KTTECH_TOUCH_BOOSTER
	if (status & (MXT_MSGB_T9_PRESS | MXT_MSGB_T9_MOVE))
		set_dvfs_lock(mxt, MXT_TOUCH_BOOST_ON);
	else
		set_dvfs_lock(mxt, MXT_TOUCH_BOOST_OFF);
#endif
...
#if KTTECH_TOUCH_BOOSTER
	mutex_init(&mxt->dvfs_lock);
	INIT_DELAYED_WORK(&mxt->work_dvfs_off, set_dvfs_off);
	INIT_DELAYED_WORK(&mxt->work_dvfs_chg, change_dvfs_lock);
