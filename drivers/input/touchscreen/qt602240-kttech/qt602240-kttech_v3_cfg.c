/*
*  qt602240_kttech_v3_cfg.c - Atmel maXTouch Touchscreen Controller
*
*  Version 0.3a
*
...
#define ATMEL_DEBUG	0901

u8 firmware_latest[] = { 
	#include "atmel_mxt224_fw_ver20.h"
	};
...
// spt_userdata_t38_t userdata_t38 = {0};
...
{ 
	...
	case MXT_SPT_GPIOPWM_T19:
		obj = (u8 *)&gpiopwm_config;
		break;

	case MXT_PROCI_GRIPSUPPRESSION_T20:
		obj = (u8 *)&gripfacesuppression_config;
		break;
	...
	case MXT_TOUCH_PROXIMITY_T23:
		obj = (u8 *)&proximity_config;
		break;

	case MXT_PROCI_ONETOUCHGESTUREPROCESSOR_T24:
		obj = (u8 *)&onetouch_gesture_config;
		break;

	case MXT_SPT_SELFTEST_T25:
		obj = (u8 *)&selftest_config;
		break;
...
int mxt_grip_face_suppression_config(struct mxt_data *mxt)
{
	struct i2c_client *client = mxt->client;
	u16 obj_addr, obj_size;
	int error;

	obj_addr = MXT_BASE_ADDR(MXT_PROCI_GRIPSUPPRESSION_T20);
	obj_size = MXT_GET_SIZE(MXT_PROCI_GRIPSUPPRESSION_T20);
	...
	gripfacesuppression_config.reserved = T20_RESERVED ;
	...
	error = mxt_write_block(client, obj_addr, obj_size, (u8 *)&gripfacesuppression_config);
	if (error < 0) { 
		dev_err(&client->dev, "[TSP] mxt_write_block failed! (%s, %d)\n", __func__, __LINE__);
		return -EIO;
	}
...
int mxt_noise_suppression_config(struct mxt_data *mxt)
{
	...
	noise_suppression_config.reserved      = T22_RESERVED;
	noise_suppression_config.reserved1     = T22_RESERVED1; 
	noise_suppression_config.gcaful1       = T22_GCAFUL1;
	noise_suppression_config.gcaful2       = T22_GCAFUL2;
	noise_suppression_config.gcafll1       = T22_GCAFLL1;
	noise_suppression_config.gcafll2       = T22_GCAFLL2;
	noise_suppression_config.actvgcafvalid = T22_ACTVGCAFVALID;
	noise_suppression_config.noisethr      = T22_NOISETHR;
	noise_suppression_config.reserved2     = T22_RESERVED2;
	noise_suppression_config.freqhopscale  = T22_FREQHOPSCALE;
	noise_suppression_config.freq[0]       = T22_FREQ0;
	noise_suppression_config.freq[1]       = T22_FREQ1;
	noise_suppression_config.freq[2]       = T22_FREQ2;
	noise_suppression_config.freq[3]       = T22_FREQ3;
	noise_suppression_config.freq[4]       = T22_FREQ4;
	noise_suppression_config.idlegcafvalid = T22_IDLECAFVALID;
...
{ 
	...
	cte_config.idlegcafdepth          = T28_IDLEGCAFDEPTH;
	cte_config.actvgcafdepth          = T28_ACTVGCAFDEPTH;
	cte_config.voltage                = T28_VOLTAGE;
...
{ 
	...
	if (mxt_grip_face_suppression_config(mxt) < 0)  /* T20 */
		return -1;

	if (mxt_noise_suppression_config(mxt) < 0)      /* T22 */
		return -1;
