...
#ifdef KTTECH_SENSOR_BMC050
static int bmm_power_state = 0;
static struct regulator *bmm_main_8058_lvs0 = NULL; //1.8V main power
extern int bma250_get_power_state(void);
#endif
...
#ifdef KTTECH_SENSOR_BMC050
int bmm_get_power_state(void)
{
	return bmm_power_state;
};
EXPORT_SYMBOL(bmm_get_power_state);

static int bmm_power_control(int on)
{
	int rc = 0;

	if(on) {
		if(bmm_power_state || bma250_get_power_state()) {
			bmm_power_state = 1;
			pr_err("%s: already main power on\n", __func__);
			return 0;
		}  

		if (!bmm_main_8058_lvs0) {
			bmm_main_8058_lvs0 = regulator_get(NULL, "8058_lvs0");

			if (IS_ERR(bmm_main_8058_lvs0)) {
				pr_err("%s: regulator_get(8058_lvs0) failed (%d)\n", __func__, rc);
				bmm_main_8058_lvs0 = NULL;
				return PTR_ERR(bmm_main_8058_lvs0);
			}
		}

		//enable output
		rc = regulator_enable(bmm_main_8058_lvs0);
		if (rc)
		{ 
			pr_err("%s: regulator_enable(bmm_main_8058_lvs0) failed (%d)\n", __func__, rc);
			regulator_put(bmm_main_8058_lvs0);
			return rc;
		}
		...
		if (bmm_main_8058_lvs0)
		{
			rc = regulator_force_disable(bmm_main_8058_lvs0);
		...
		u8 *data, u8 len)
{
	...
	while (0 != len--) {
#ifdef BMM_SMBUS
		for (retry = 0 ; retry < BMM_I2C_RETRY ; retry++) {
		...
		if (retry >= BMM_I2C_RETRY) {
			printk(KERN_ERR "%s: retry over %d : error[%d]\n",__func__, BMM_I2C_RETRY, ret);
			return ret;
		} else
			*data = (u8)(ret & 0xff);
#else
		...
		const char *buf, size_t count)
{
	...
	if (i != -1) {
#ifdef KTTECH_SENSOR_BMC050
		if ((op_mode != client_data->op_mode) && (BMM_VAL_NAME(SUSPEND_MODE) != op_mode)) {
			if(!bmm_get_power_state()) {
				bmm_power_control(1);
				mdelay(BMM_I2C_WRITE_DELAY_TIME);
