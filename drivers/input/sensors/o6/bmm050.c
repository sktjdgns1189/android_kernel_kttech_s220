...
#ifdef KTTECH_SENSOR_BMC050
static int bmm_charger_offset_x = 0;
static int bmm_charger_offset_y = 0;
static int bmm_charger_offset_z = 0;

void bmm_inform_charger_connection(int mode, int connected)
{
	if(connected == 1) {
		if(mode == 1) { /* Wall Charger */
			bmm_charger_offset_x = -15;
			bmm_charger_offset_y = 40;
			bmm_charger_offset_z = 30;
		} else if(mode == 0) { /* USB Charger */
			bmm_charger_offset_x = -10;
			bmm_charger_offset_y = 20;
			bmm_charger_offset_z = 20;
		}
	} else {
		bmm_charger_offset_x = 0;
		bmm_charger_offset_y = 0;
		bmm_charger_offset_z = 0;
	}
};
EXPORT_SYMBOL(bmm_inform_charger_connection);
#endif
...
#ifdef KTTECH_SENSOR_BMC050
		mdata->datax = mdata->datax + bmm_charger_offset_x;
		mdata->datay = mdata->datay + bmm_charger_offset_y;
		mdata->dataz = mdata->dataz + bmm_charger_offset_z;
#endif		
	}
	return comres;
}
