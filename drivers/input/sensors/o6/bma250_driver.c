...
#ifdef KTTECH_SENSOR_BMC050
#include <mach/gpio.h>
#include <linux/regulator/consumer.h>
#include <linux/miscdevice.h>

#define BMA250_I2C_RETRY 2
#define BMA250_DEVICE_FILE_NAME "bma250_dev"
#endif
...
#ifdef KTTECH_SENSOR_BMC050
static struct i2c_client * bma250_client_backup = 0;
static int bma250_current_mode = 0;
#endif
...
#ifdef KTTECH_SENSOR_BMC050
static int bma250_power_state = 0;
static struct regulator *bma250_main_8058_lvs0 = NULL; //1.8V main power
extern int bmm_get_power_state(void);
#endif

#ifdef KTTECH_SENSOR_BMC050
int bma250_get_power_state(void)
{
	return bma250_power_state;
};
EXPORT_SYMBOL(bma250_get_power_state);

static int bma250_power_control(int on)
{
	int rc = 0;

	if(on) {
		if(bma250_power_state || bmm_get_power_state()) {
			bma250_power_state = 1;
			pr_err("%s: already main power on\n", __func__);
			return 0;
		}  

		if (!bma250_main_8058_lvs0) {
			bma250_main_8058_lvs0 = regulator_get(NULL, "8058_lvs0");

			if (IS_ERR(bma250_main_8058_lvs0)) {
				pr_err("%s: regulator_get(8058_lvs0) failed (%d)\n", __func__, rc);
				bma250_main_8058_lvs0 = NULL;
				return PTR_ERR(bma250_main_8058_lvs0);
			}
		}

		//enable output
		rc = regulator_enable(bma250_main_8058_lvs0);
		if (rc)
		{ 
			pr_err("%s: regulator_enable(bma250_main_8058_lvs0) failed (%d)\n", __func__, rc);
			regulator_put(bma250_main_8058_lvs0);
			return rc;
		}
		
		gpio_tlmm_config(GPIO_CFG(115, 2, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_16MA), GPIO_CFG_ENABLE);
		gpio_tlmm_config(GPIO_CFG(116, 2, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_16MA), GPIO_CFG_ENABLE);
		msleep(5);
		bma250_power_state = 1;

		printk("bma250_power_control on\n");
	}
	else {
		if(!bma250_power_state || bmm_get_power_state()) {
			bma250_power_state = 0;
			pr_err("%s: already main power off\n", __func__);
			return 0;
		}

		if (bma250_main_8058_lvs0)
		{
			rc = regulator_force_disable(bma250_main_8058_lvs0);

			if (rc) { 
				pr_err("%s: regulator_disable(8058_lvs0a) failed (%d)\n", __func__, rc);
			}
			regulator_put(bma250_main_8058_lvs0);

			bma250_main_8058_lvs0 = NULL;
		}
		gpio_tlmm_config(GPIO_CFG(115, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), GPIO_CFG_ENABLE);
		gpio_tlmm_config(GPIO_CFG(116, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), GPIO_CFG_ENABLE);
		bma250_power_state = 0;

		printk("bma250_power_control off\n");
	}
	return 0;
}
		...
		unsigned char reg_addr, unsigned char *data)
{
	...
	for (retry = 0 ; retry < BMA250_I2C_RETRY ; retry++) {
	...
	if (retry >= BMA250_I2C_RETRY) {
		printk(KERN_ERR "%s: retry over %d : error[%d]\n",__func__, BMA250_I2C_RETRY, ret);
		return ret;
	} else {
		*data = ret & 0x000000ff;
		return 0;
...
static int bma250_set_mode(struct i2c_client *client, unsigned char Mode)
{
	...
	if (client == NULL) {
		...
		if (Mode < 3) {
#ifdef KTTECH_SENSOR_BMC050			
			if (!bma250_get_power_state() && (Mode == BMA250_MODE_SUSPEND))
				return comres;

			if (Mode != BMA250_MODE_SUSPEND) {
				bma250_power_control(1);
				if (Mode == BMA250_MODE_NORMAL) {
					unsigned char reset;
					reset = 0xB6;
					bma250_smbus_write_byte(bma250_client_backup, BMA250_RESET_REG, &reset);
					mdelay(100);
				}
				
			}

			bma250_current_mode = Mode;
#endif
...
static ssize_t bma250_xyz_cal_data_read(struct file *filp, char __user *ubuf, size_t cnt, loff_t *ppos)
{
	unsigned char offset_x = 0;
	unsigned char offset_y = 0;
	unsigned char offset_z = 0;

	if (cnt < 12)
		return -EINVAL;

	if (bma250_get_offset_filt_x(bma250_client_backup, &offset_x) < 0)
		printk("bma250_get_offset_filt_x read error\n");
	...
	if (bma250_get_offset_filt_y(bma250_client_backup, &offset_y) < 0)
		printk("bma250_get_offset_filt_y read error\n");
	...
	if (bma250_get_offset_filt_z(bma250_client_backup, &offset_z) < 0)
		printk("bma250_get_offset_filt_z read error\n");
...
static ssize_t bma250_xyz_cal_enable_write(struct file *filp, const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	if (cnt != 1)
		return -EINVAL;

	if (ubuf[0] == 1) {
		unsigned char bma250_mode;

		bma250_mode = bma250_current_mode; 
		
		if(bma250_mode != BMA250_MODE_NORMAL) {
			if (bma250_set_mode(bma250_client_backup, BMA250_MODE_NORMAL) < 0)
				printk(KERN_INFO "bma250_set_mode FAIL\n");

			mdelay(100);
		}

		if (bma250_set_offset_target_x(bma250_client_backup,
					(unsigned char)2) < 0)
			return -EINVAL;

		if (bma250_set_cal_trigger(bma250_client_backup, 1) < 0)
			return -EINVAL;

		mdelay(300);

		if (bma250_set_offset_target_y(bma250_client_backup,
					(unsigned char)2) < 0)
			return -EINVAL;

		if (bma250_set_cal_trigger(bma250_client_backup, 2) < 0)
			return -EINVAL;

		mdelay(300);

		if (bma250_set_offset_target_z(bma250_client_backup,
					(unsigned char)0) < 0)
			return -EINVAL;

		if (bma250_set_cal_trigger(bma250_client_backup, 3) < 0)
			return -EINVAL;
		...
		/* unlock eeprom */
		if (bma250_set_ee_w(bma250_client_backup, 1) < 0)
			return -EINVAL;

		mdelay(200);

		if (bma250_set_ee_prog_trig(bma250_client_backup) < 0)
			return -EINVAL;

		mdelay(500);

		/* unlock eeprom */
		if (bma250_set_ee_w(bma250_client_backup, 0) < 0)
			return -EINVAL;

		if (bma250_mode == BMA250_MODE_LOWPOWER) {
			if (bma250_set_mode(bma250_client_backup, BMA250_MODE_LOWPOWER) < 0)
				printk(KERN_INFO "bma250_set_mode BMA250_MODE_LOWPOWER FAIL\n");
		} else if (bma250_mode == BMA250_MODE_SUSPEND) {
			if (bma250_set_mode(bma250_client_backup, BMA250_MODE_SUSPEND) < 0)
				printk(KERN_INFO "bma250_set_mode BMA250_MODE_LOWPOWER FAIL\n");
		}
		printk(KERN_INFO "bma250 fast calibration finished\n");
	}	
	return 1;
}
...
static struct file_operations bma250_fops = {
	...
	.read = bma250_xyz_cal_data_read,
	.write = bma250_xyz_cal_enable_write,
};


static struct miscdevice bma250_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = BMA250_DEVICE_FILE_NAME,
	.fops = &bma250_fops,
};
...
#ifdef KTTECH_SENSOR_BMC050	
	bma250_client_backup = client;
	if (misc_register(&bma250_device)) {
		printk(KERN_ERR "!!! bma250_probe: misc failed\n");
	}
#endif	
