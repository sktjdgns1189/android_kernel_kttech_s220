...
#define T3900_PLAY_IF_DELAY				1000 // 500 test value
#define T3900_PLAY_RF_DELAY				1100 // 800 test value
...
#define INC_INTERRUPT_SIZE				(188*16)//(192*16*8)
...
#define INC_ENSEMBLE_LABEL_MAX				17
...
typedef struct _tagST_FICDB_SERVICE_COMPONENT
{
	...
	/************************************************************************/
	INC_UINT8	aucComponentLabels[INC_ENSEMBLE_LABEL_MAX];
	INC_UINT8	ucIsComponentLabel;

	/************************************************************************/
	/* Type 1 of Extension 1,5 field                                        */
	/************************************************************************/
	INC_UINT8	aucLabels[INC_ENSEMBLE_LABEL_MAX];
...
typedef struct _tagST_FICDB_LIST
{
	...
	INC_UINT8		aucEnsembleName[INC_ENSEMBLE_LABEL_MAX];
	INC_UINT16		unEnsembleFlag;
...
//#define INC_DEBUG_MSG

#ifdef INC_DEBUG_MSG
void 		INC_MSG_PRINTF(INC_INT8 nFlag, INC_INT8* pFormat, ...);
#else
#define INC_MSG_PRINTF(nFlag, pFormat, ...)
#endif
