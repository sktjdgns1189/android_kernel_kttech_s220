...
#if 0 //def CONFIG_KTTECH_WIFI_BCM4329
#include <linux/platform_device.h>
...
#ifdef CONFIG_BCM4329_MEM_PREALLOC
extern void *bcm4329_wifi_mem_prealloc(int section, unsigned long size);
#endif
/*
...
#if 0 //def CONFIG_KTTECH_WIFI_BCM4329
static int dhd_readwrite_file(const char *filename, char *rbuf, const char *wbuf, unsigned int length)
{
    int ret = 0;
    ...
    do {
        ...
        if (wbuf) {
            if ( (ret=filp->f_op->write(filp, wbuf, length, &filp->f_pos)) < 0) {
				printk("%s, Write bytes to file error= %d  \n", __FUNCTION__, ret);
                break;
            }
        } else {
            if ( (ret=filp->f_op->read(filp, rbuf, length, &filp->f_pos)) < 0) {
				printk("%s, Read bytes to file error= %d  \n", __FUNCTION__, ret);
                break;
            }
        }
    } while (0);
...
static void dhd_enable_mmchost_detect_change(int enable)
{
#define MMC_MSM_DEV "msm_sdcc.4"
    char buf[3];
    int length;
    length = snprintf(buf, sizeof(buf), "%d\n", enable ? 1 : 0);
    if (dhd_readwrite_file("/sys/devices/platform/" MMC_MSM_DEV "/detect_change", 
                               NULL, buf, length) < 0) {
		printk("%s: dhd_enable_mmchost_detect_change... \n", __FUNCTION__);

        /* fall back to polling */
        dhd_readwrite_file("/sys/devices/platform/" MMC_MSM_DEV "/polling", NULL, buf, length);
    }
}
#endif
...
dhd_module_cleanup(void)
{
	...
	dhd_bus_unregister();
#if (defined(CUSTOMER_HW2) && defined(CONFIG_WIFI_CONTROL_FUNC)) // || defined(CONFIG_KTTECH_WIFI_BCM4329)
	wifi_del_dev();
#endif
...
#if 0 //def CONFIG_KTTECH_WIFI_BCM4329
	dhd_enable_mmchost_detect_change(0);
#endif
...
dhd_module_init(void)
{
	...
	if (!error)
		...
		printf("sdio_register_driver failed\n");		
	...
	if (down_timeout(&dhd_registration_sem,  msecs_to_jiffies(DHD_REGISTRATION_TIMEOUT)) != 0) {
		error = -EINVAL;
		printf("sdio_register_driver timeout start \n");
		dhd_bus_unregister();
		printf("sdio_register_driver timeout end \n");
		  
		goto faild;		
	}
#endif
	printk("%s: platform_driver_dhd init... error = %d  \n", __FUNCTION__, error);

	return error;

faild:
	printk("%s: platform_driver_dhd init... faild \n", __FUNCTION__);
...
void * dhd_os_prealloc(int section, unsigned long size)
{
    ...
    alloc_ptr = bcm4329_wifi_mem_prealloc(section, size);
    if (alloc_ptr)
    {
      DHD_INFO(("success alloc section %d\n", section));
      bzero(alloc_ptr, size);
...
#ifdef CONFIG_KTTECH_WIFI_BCM4329
void
wl_set_romm_parms( struct net_device *dev )
{
