...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	*/
	...
	if (dhd->country_code[0] != 0) {
		if ((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_COUNTRY,
	...
	if (dhd_roam == 0)
	{
		...
		if ((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_ROAM_SCAN_PERIOD, \
		...
		if ((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, \
		...
		if (dhdcdc_query_ioctl(dhd, 0, WLC_GET_BAND, \
			...
			if ((band == WLC_BAND_AUTO) || (band == WLC_BAND_ALL))
			{
				...
				if ((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_BAND, \
					(char *)&band_temp_set, sizeof(band_temp_set))) < 0)
					DHD_ERROR(("%s: local band seting failed\n", __FUNCTION__));
			}
			if ((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_ROAM_DELTA, \
				(char *)&roam_delta, sizeof(roam_delta))) < 0)
				DHD_ERROR(("%s: roam delta setting failed\n", __FUNCTION__));

			if ((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_ROAM_TRIGGER, \
	...
	/* Force STA UP */
	if (dhd_radio_up)
		ret = dhdcdc_set_ioctl(dhd, 0, WLC_UP, (char *)&up, sizeof(up));
	...
	ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_SCAN_CHANNEL_TIME, (char *)&scan_assoc_time,
		sizeof(scan_assoc_time));
	ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_SCAN_UNASSOC_TIME, (char *)&scan_unassoc_time,
		sizeof(scan_unassoc_time));

#ifdef CONFIG_KTTECH_WIFI_BCM4329
   ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_SCAN_HOME_TIME, (char *)&scan_home_time,
   ...
   dhd->keep_alive.period_msec = 30000; /*  jaehyungkim@kttech.co.kr  keep alive packet: 30sec */

	if(dhd->keep_alive.period_msec > 0) {
		int buf_len;
		char pktdata[] = "null_pkt";
		char kbuf[32];
		wl_keep_alive_pkt_t *ppkt = (wl_keep_alive_pkt_t *)kbuf;

		ppkt->period_msec = dhd->keep_alive.period_msec;
		ppkt->len_bytes = strlen(pktdata);
		memcpy(ppkt->data, pktdata, ppkt->len_bytes);

		buf_len = bcm_mkiovar("keep_alive",
			kbuf,
			WL_KEEP_ALIVE_FIXED_LEN + ppkt->len_bytes,
			buf, sizeof(buf));

		if (dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, buf, buf_len)< 0)
		{
