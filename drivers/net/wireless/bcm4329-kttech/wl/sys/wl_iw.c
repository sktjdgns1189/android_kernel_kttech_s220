...
static int      g_roamonoff = 0; // 1:roam offf, 0:roam on
#endif
...
#ifdef CONFIG_KTTECH_WIFI_BCM4329
static int wl_iw_set_roam_mode(
	struct net_device *dev,
	struct iw_request_info *info,
	union iwreq_data *wrqu,
	char *extra
)
{
   ...
   uint dhd_roam_off = 1;
	...
	strncpy((char *)&raommode_val, extra + strlen("ROAM") +1, 1);

   if (strnicmp((char *)&raommode_val, "1", strlen("1")) == 0) {
       g_roamonoff = 0;
		bcm_mkiovar("roam_off", (char *)&dhd_roam_on, 4, iovbuf, sizeof(iovbuf));
		dev_wlc_ioctl(dev, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
		WL_TRACE(("%s: ROAM on \n", __FUNCTION__));		
                wl_set_romm_parms(dev);
      
	}
	else if (strnicmp((char *)&raommode_val, "0", strlen("0")) == 0) {
       g_roamonoff = 1;
	   ...
	   dev_wlc_ioctl(dev, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
		...
		WL_ERROR(("%s Unkwown yet roam setting, ignored\n",__FUNCTION__));
	}

   WL_ERROR(("%s  ROAM ON/OFF :%d\n",__FUNCTION__,g_roamonoff));
...
)
{
	...
	p += snprintf(p, MAX_WX_STRING, "GETROAM %d", g_roamonoff);
	wrqu->data.length = p - extra + 1;
...
)
{
	...
	WL_ERROR(("wl_iw_get_power_mode result:%d\n", pm));
	...
	p += snprintf(p, MAX_WX_STRING, "GETPOWER %d", pm_up);
	wrqu->data.length = p - extra + 1;
...
)
{
	...
	ppkt = (wl_keep_alive_pkt_t *)buf;
	ppkt->period_msec = bcm_atoi(buf_period);

   WL_ERROR(("%s: period_msec:%d \n", __FUNCTION__,ppkt->period_msec));

	if ( ppkt->period_msec > 0 )
	{
		char pktdata[] = "null_pkt";

		ppkt->len_bytes = strlen(pktdata);
		memcpy(ppkt->data, pktdata, ppkt->len_bytes);
		buflen = WL_KEEP_ALIVE_FIXED_LEN + ppkt->len_bytes;
	}
	else
	{
		ppkt->len_bytes = 0;
		buflen = sizeof(wl_keep_alive_pkt_t);
	}

	if((error = dev_wlc_bufvar_set(dev, "keep_alive", buf, buflen)) < 0) {
...
#ifdef CONFIG_KTTECH_WIFI_BCM4329 // SoftAP 
...
)
{
  ...
  value = dtoh32(ci.hw_channel);

  WL_TRACE(("%s: wl_iw_get_curr_freq :%d\n", dev->name, value));

  p += snprintf(p, MAX_WX_STRING, "GETCH %d", value);
...
_iscan_sysioc_thread(void *data)
{
	...
	if  (g_scan_specified_ssid && (iscan_pass_abort == TRUE)) {
		printk("%s Get results from specific scan status=%d\n", __FUNCTION__, status);
...
)
{
   ...
   char *p = extra;
	...
	{
		value = DIV_QUO(power.est_Pout[0], 4);
	}

	WL_TRACE(("%s: wl_iw_get_txpower :%d\n", dev->name, value));

	p += snprintf(p, MAX_WX_STRING, "TXPOWER %d", value);
	wrqu->data.length = p - extra + 1;
...
)
{
	...
	if (dwrq->length && extra) {
       ...
       else if (strnicmp(extra, "POWERSAVE", strlen("POWERSAVE")) == 0)
			ret = wl_iw_set_power_mode(dev, info, (union iwreq_data *)dwrq, extra);
       else if (strnicmp(extra, "ROAM", strlen("ROAM")) == 0 )
         ret = wl_iw_set_roam_mode(dev, info, (union iwreq_data *)dwrq, extra);
       else if (strnicmp(extra, "GETROAM", strlen("GETROAM")) == 0 )
         ret = wl_iw_get_roam_mode(dev, info, (union iwreq_data *)dwrq, extra);
       else if (strnicmp(extra, "GETPOWER", strlen("GETPOWER")) == 0 )
         ret = wl_iw_get_power_mode(dev, info, (union iwreq_data *)dwrq, extra);    
       else if (strnicmp(extra, "TXPOWER", strlen("TXPOWER")) == 0 )
          ret = wl_iw_get_txpower(dev, info, (union iwreq_data *)dwrq, extra);       
       else if (strnicmp(extra, "GETCH", strlen("GETCH")) == 0 )
         ret = wl_iw_get_curr_freq(dev, info, (union iwreq_data *)dwrq, extra);       
#endif
#ifdef SOFTAP
