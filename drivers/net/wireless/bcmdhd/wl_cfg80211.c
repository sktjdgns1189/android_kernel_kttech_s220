	...
	struct bss_parameters *params)
{
	...
	{
      WL_DBG(("P:use_cts_proto[%d]\n", params->use_cts_prot));
	}

	if (params->use_short_preamble >= 0)
	{
      WL_DBG(("P:use_short_preamble[%d]\n", params->use_short_preamble));
	}

	if (params->use_short_slot_time >= 0)
	{
      WL_DBG(("P:use_short_slot_time[%d]\n", params->use_short_slot_time));
	}

	if (params->basic_rates)
	{
		for(val=0; val<params->basic_rates_len; val++)
		{
            WL_DBG(("P:basic_rates[%d]:[%d]\n", val, params->basic_rates[val]));
		}
		nmode = 0;
  ...
  struct net_device *dev, char *command, int total_len)
{
  ...
  if (unlikely(err)) {
    ...
    qdbm = (u8) (txpwrdbm & ~WL_TXPWR_OVERRIDE);
    value = DIV_QUO(qdbm, 4);
  }

  bytes_written = snprintf(command, total_len, "TXPOWER1 %d", value);
  WL_DBG(("%s: command result is %s\n", __FUNCTION__, command));

  return bytes_written;
}

int wl_cfg80211_get_fer_per(
  struct net_device *dev, char *command, int total_len)
{
  int error = 0;
  get_pktcnt_t pktcnt;
  ...
  if((error = wldev_ioctl(dev, WLC_GET_PKTCNTS, &pktcnt, sizeof(pktcnt), 0))<0){
    WL_ERR(("%s: get packet_info error (%d)\n", __FUNCTION__, error));
  } 
  else {
  ...
  bytes_written = snprintf(command, total_len, "FER,PER1 %u", pkt_rate);
  WL_DBG(("%s: command result is %s\n", __FUNCTION__, command));
  
  return bytes_written;
}

int wl_cfg80211_get_curr_freq(
  struct net_device *dev, char *command, int total_len)
{
  int error = 0;
  channel_info_t ci;
  ...
  if((error = wldev_ioctl(dev, WLC_GET_CHANNEL, &ci, sizeof(ci), 0))<0){
    WL_ERR(("%s: get curr_freq error (%d)\n", __FUNCTION__, error));
    value = 0;
  }
  else {
    value = dtoh32(ci.hw_channel);  
    bytes_written = snprintf(command, total_len, "GETCH1 %d", value);
  }
  ...
  struct net_device *dev, char *command, int total_len)
{
  ...
  int bytes_written = 0;
	...
	strncpy((char *)&raommode_val, command + strlen("ROAM1") +1, 1);
  
  if (strnicmp((char *)&raommode_val, "1", strlen("1")) == 0) {
    g_roamonoff = 0;    
    bcm_mkiovar("roam_off", (char *)&dhd_roam_on, 4, iovbuf, sizeof(iovbuf));
    error = wldev_ioctl(dev, WLC_SET_VAR, iovbuf, sizeof(iovbuf), true);
    if (unlikely(error))
      WL_ERR(("%s: ROAM on failed\n", __FUNCTION__));
    else 
      WL_TRACE(("%s: ROAM on \n", __FUNCTION__));		
    ...
    error = wldev_ioctl(dev, WLC_SET_VAR, iovbuf, sizeof(iovbuf), true);
    if (unlikely(error)) 
      WL_ERR(("%s: ROAM off failed\n", __FUNCTION__));
    else 
     WL_TRACE(("%s: ROAM off \n", __FUNCTION__));	
  }
  else {
    WL_ERR(("%s Unkwown yet roam setting, ignored\n",__FUNCTION__));
  }
  ...
  struct net_device *dev, char *command, int total_len)
{
  ...
  bytes_written = snprintf(command, total_len, "GETROAM1 %d", g_roamonoff);
