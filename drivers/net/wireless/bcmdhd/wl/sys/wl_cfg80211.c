	...
	if (status == WLC_E_STATUS_PARTIAL) {
		...
		if (!(wl_to_wiphy(wl)->interface_modes & BIT(NL80211_IFTYPE_ADHOC))) {
			if (dtoh16(bi->capability) & DOT11_CAP_IBSS) {
				WL_TRACE(("Ignoring IBSS result\n")); // KTTECH replace... WL_ERR(("Ignoring IBSS result\n"));
				goto exit;
			}
		}
...
s32 wl_cfg80211_up(void *para)
{
	...
	ioctl_version = val;
	WL_INFO(("WLC_GET_VERSION=%d\n", ioctl_version)); //KTTECH replacement... WL_ERR(("WLC_GET_VERSION=%d\n", ioctl_version));
  ...
  struct net_device *dev, char *command, int total_len)
{
  ...
  bytes_written = snprintf(command, total_len, "TXPOWER %d", value);
  ...
  struct net_device *dev, char *command, int total_len)
{
  ...
  bytes_written = snprintf(command, total_len, "FER,PER %u", pkt_rate);
  
  //WL_ERR(("%s: command result is %s\n", __FUNCTION__, command));
  ...
  struct net_device *dev, char *command, int total_len)
{
  ...
  if((error = wldev_ioctl(dev, WLC_GET_CHANNEL, &ci, sizeof(ci), 0))<0){
    ...
    bytes_written = snprintf(command, total_len, "GETCH %d", value);
  }
  ...
  struct net_device *dev, char *command, int total_len)
{
  ...
  int bytes_written = 0;
	...
	strncpy((char *)&raommode_val, command + strlen("ROAM") +1, 1);
  ...
  struct net_device *dev, char *command, int total_len)
{
  ...
  bytes_written = snprintf(command, total_len, "GETROAM %d", g_roamonoff);
