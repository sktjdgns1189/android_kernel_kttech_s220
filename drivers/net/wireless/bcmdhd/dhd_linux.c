...
#ifdef CONFIG_KTTECH_WIRELESS_BCM4330
		if ((ifidx == 0) && (m_dhd_op_mode == DHD_FLAG_CONCURR_SINGLE_CHAN_MODE))
#else
...
dhd_open(struct net_device *net)
{
	...
	if (ifidx == 0) {
		...
		DHD_ERROR(("\n%s\n", dhd_version));
#ifdef CONFIG_KTTECH_WIRELESS_BCM4330
        if(dhd->pub.op_mode == DHD_FLAG_CONCURR_SINGLE_CHAN_MODE)
#else
...
#if 0//def CONFIG_KTTECH_WIRELESS_BCM4330 // Because of the POWER OFF  already DHD_STOP()...
    if(m_dhd_op_mode != DHD_FLAG_CONCURR_SINGLE_CHAN_MODE)
