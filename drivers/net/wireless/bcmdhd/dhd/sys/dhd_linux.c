...
extern void *bcm4330_wifi_mem_prealloc(int section, unsigned long size);
#endif
...
dhd_sendpkt(dhd_pub_t *dhdp, int ifidx, void *pktbuf)
{
	...
	if (PKTPRIO(pktbuf) == 0 || PKTPRIO(pktbuf) == 5)
#else  
	if (PKTPRIO(pktbuf) == 0)
#endif    
...
#ifdef CONFIG_KTTECH_WIRELESS_BCM4330
    if ((ifidx == 0) && (m_dhd_op_mode == STA_MASK))
#else
...
#ifndef CONFIG_KTTECH_WIRELESS_BCM4330 //STA vs WFD for distinction
...
// shjin (2012.02.22) - merge s300 code
// S=====================================
#ifdef CONFIG_KTTECH_WIRELESS_BCM4330
	int scan_assoc_time = 120;
	int scan_unassoc_time = 80;
	//int scan_home_time = 120;
	//int wsec_param = 0;
#else
...
#ifdef CONFIG_KTTECH_WIRELESS_BCM4330 //STA vs WFD for distinction
...
#ifdef CONFIG_KTTECH_WIRELESS_BCM4330
    m_dhd_op_mode = dhd->op_mode; //Backup the DHD OP_MODE for Power_ctl... 
#endif
...
// 1. Set scan param
#ifdef CONFIG_KTTECH_WIRELESS_BCM4330
//    dhd_wl_ioctl_cmd(dhd, WLC_SET_SCAN_HOME_TIME, (char *)&scan_home_time,
//        sizeof(scan_home_time), TRUE, 0);
#endif

// 2. Set roaming param
#ifdef CONFIG_KTTECH_WIRELESS_BCM4330	
  if (dhd_roam_disable == 0)
  {
...
// 3. broadcom patch
#ifdef CONFIG_KTTECH_WIRELESS_BCM4330	
//	bcm_mkiovar("ht_wsec_restrict", (char *)&wsec_param, 4, iovbuf, sizeof(iovbuf));  
//	dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
#endif 
// E========================================================================
...
#ifdef CONFIG_KTTECH_WIRELESS_BCM4330
/* Set keep_alive */
   dhd->mkeep_alive_pkt.period_msec = 30000; /* keep alive packet: 30sec */
   
   if(dhd->mkeep_alive_pkt.period_msec > 0) {
     int buf_len;
     int str_len;
     ...
     mkeep_alive_pkt.period_msec =  dhd->mkeep_alive_pkt.period_msec;
     buf_len = str_len + 1;
     ...
     {
       DHD_ERROR(("%s:keep alive success \n", __FUNCTION__));
     }
   }
#endif
// E========================================================================
...
#ifdef CONFIG_KTTECH_WIRELESS_BCM4330 // Because of the POWER OFF  already DHD_STOP()...
    if(m_dhd_op_mode != STA_MASK)
...
#if 0//def CONFIG_KTTECH_WIRELESS_BCM4330  
...
uint8* dhd_os_prealloc(void *osh, int section, uint size)
{
    ...
    alloc_ptr = bcm4330_wifi_mem_prealloc(section, size);
    if (alloc_ptr)
    {
...
#ifndef CONFIG_KTTECH_WIRELESS_BCM4330 // IF up/down ready ICS Frameworks it work....
			dev_close(dev);
			dev_open(dev);
#endif			
#endif
