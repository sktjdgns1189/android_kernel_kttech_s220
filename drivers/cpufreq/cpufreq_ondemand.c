...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (max_load_freq > dbs_tuners_ins.up_threshold * policy->cur) {
		...
		if (!dbs_tuners_ins.io_is_busy && policy->cur < LOW_MIN_FREQ_LIMIT && policy->max > LOW_MAX_FREQ_LIMIT)
			dbs_freq_increase(policy, LOW_MAX_FREQ_LIMIT);
		else
#else
...
#ifdef CONFIG_KTTECH_DVFS
	if (policy->max > IO_MAX_FREQ_LIMIT && policy->cur < IO_MAX_FREQ_LIMIT) {
		policy->cur = IO_MAX_FREQ_LIMIT;

		__cpufreq_driver_target(policy, IO_MAX_FREQ_LIMIT,
					CPUFREQ_RELATION_L);
