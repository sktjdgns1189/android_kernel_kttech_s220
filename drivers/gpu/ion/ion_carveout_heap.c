...
static int sf_err_count = 0;
static int sf_err_max_size = 0;	
static int camera_err_count = 0;
static int camera_err_max_size = 0;
void ion_carveout_kttech_print_debug(void);

void ion_carveout_kttech_print_debug(void)
{
	if(sf_err_count) {
		pr_err("## Memory allocation failed. (sf : %d, 0x%x)\n", sf_err_count, sf_err_max_size);
	}

	if(camera_err_count) {
		pr_err("## Memory allocation failed. (camera : %d, 0x%x)\n", camera_err_count, camera_err_max_size);
	}
}
#endif
				      ...
				      unsigned long align)
{
	...
	if (!offset) {
#ifdef CONFIG_KTTECH_MODEL_O4
		pr_err("%s: heap %s can not allocate memory. size(0x%lx)\n",
			__func__, heap->name, size);
		switch(heap->id) {
		case ION_SF_HEAP_ID:
			{
				sf_err_count++;
				if(sf_err_max_size < size)
					sf_err_max_size = size;
			}
			break;
		case ION_CAMERA_HEAP_ID:
			{
				camera_err_count++;
				if(camera_err_max_size < size)
					camera_err_max_size = size;
			}
			break;
		}
#else
