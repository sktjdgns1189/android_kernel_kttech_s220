...
static int wb_err_count = 0;
static int wb_err_max_size = 0;	

void ion_cp_kttech_print_debug(void);

void ion_cp_kttech_print_debug(void)
{
	if(wb_err_count) {
		pr_err("## Memory allocation failed. (wb : %d, 0x%x)\n", wb_err_count, wb_err_max_size);
	}
}
#endif
				      ...
				      unsigned long flags)
{
	...
	if (!offset) {
		...
		wb_err_count ++;
		if(wb_err_max_size < size)
			wb_err_max_size = size;
#else
		if ((cp_heap->total_size -
		      cp_heap->allocated_bytes) > size)
			pr_debug("%s: heap %s has enough memory (%lx) but"
