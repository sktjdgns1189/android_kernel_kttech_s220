...
#ifdef CONFIG_MACH_KTTECH
extern int FB_UNBLANK_FLAG;

#ifdef CONFIG_KTTECH_NOVATEK_BACKLIGHT
/* For O6 PWM BL Ctrl */
static int fb_backlight_level = 10;
static int fb_fast_backlight_valid_check = 0;
#endif /* CONFIG_KTTECH_NOVATEK_BACKLIGHT */
#endif

#ifdef CONFIG_KTTECH_NOVATEK_BACKLIGHT
static char led_pwm1[2] = {0x51, 0x50};	/* DTYPE_DCS_WRITE1 */
static char led_pwm2[2] = {0x53, 0x24}; /* DTYPE_DCS_WRITE1 */
/*
    1. UI mode[power saving:10% under]
    2. Still mode[30% over]
    3. Moving mode[30% over]
*/
...
#define MIPI_NOVATEK_GAMMA_DEFAULT
//#define MIPI_NOVATEK_GAMMA_PLUS_0_2
//#define MIPI_NOVATEK_GAMMA_MINUS_0_2

/* Gamma Default */
#ifdef MIPI_NOVATEK_GAMMA_DEFAULT
static struct dsi_cmd_desc novatek_cmd_on_cmds[] = {
	{DTYPE_DCS_WRITE, 1, 0, 0, 25,
		sizeof(sw_reset), sw_reset},
...
/* Gamma Plus 0.2 */
#ifdef MIPI_NOVATEK_GAMMA_PLUS_0_2
#include "mipi_novatek_gamma_0.2p.c"
#endif

/* Gamma Minus 0.2 */
#ifdef MIPI_NOVATEK_GAMMA_MINUS_0_2
#include "mipi_novatek_gamma_0.2m.c"
#endif
...
static int boot_delay_bl_on_flag = 1;

void boot_delay_bl_on(struct work_struct *w)
{
	struct msm_fb_data_type *mfd = container_of(w, struct msm_fb_data_type, boot_bl_on.work);
	static int pwm_step;

	mutex_lock(&mfd->dma->ov_mutex);
	if (mfd) {
		if (fb_backlight_level > 0) {
			printk("\n!!!!!!!!!!!boot delay: fb_backlight_level[ %d] \n", fb_backlight_level);
			pwm_step = (fb_backlight_level*(PWM_MAX*100/mfd->panel_info.bl_max))/100;
			led_pwm1[1]  = pwm_step;
			mipi_dsi_cmds_tx(mfd, &novatek_tx_buf, novatek_cmd_backlight_cmds,
				ARRAY_SIZE(novatek_cmd_backlight_cmds));
			boot_delay_bl_on_flag =0;
		}
		else {
			printk("\n!!!!!!!!!!boot_delay_bl_on \n");
			pwm_step = (DEFAULT_BL*(PWM_MAX*100/mfd->panel_info.bl_max))/100;
			led_pwm1[1] = pwm_step;
			mipi_dsi_cmds_tx(mfd, &novatek_tx_buf, novatek_cmd_backlight_cmds,
				ARRAY_SIZE(novatek_cmd_backlight_cmds));
			boot_delay_bl_on_flag =0;
...
#ifdef CONFIG_KTTECH_NOVATEK_BACKLIGHT
	if (boot_delay_bl_on_flag == 1)
		INIT_DELAYED_WORK(&mfd->boot_bl_on, boot_delay_bl_on);
#endif
...
#ifdef CONFIG_KTTECH_NOVATEK_BACKLIGHT
		if (boot_delay_bl_on_flag == 1) {
			if (delayed_work_pending(&mfd->boot_bl_on))
				cancel_delayed_work(&mfd->boot_bl_on);
			schedule_delayed_work(&mfd->boot_bl_on, msecs_to_jiffies(300));
		}
#endif
	} else {
...
static void mipi_novatek_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	printk("\n\n### mipi_novatek_set_backlight : fast[ %d ], current [%d]\n", fb_backlight_level, mfd->bl_level);

    if(mfd->bl_level > 0 && mfd->bl_level != DEFAULT_DIM)
    {
      printk("### fb_fast_backlight_valid_check\n");
      fb_fast_backlight_valid_check = 1;  
    }
    else
    {
      fb_fast_backlight_valid_check = 0;  
    }
	...
	if (FB_UNBLANK_FLAG == 0) {
        printk("!!!mipi_novatek_set_backlight : DSI[%d] Unblank[%d]\n", mdp4_overlay_dsi_state_get(), FB_UNBLANK_FLAG);
		return;
	}

  	mutex_lock(&mfd->dma->ov_mutex);
	pwm_step = (mfd->bl_level*(PWM_MAX*100/mfd->panel_info.bl_max))/100;
	/* mdp4_dsi_cmd_busy_wait: will turn on dsi clock also */
	mdp4_dsi_cmd_dma_busy_wait(mfd);
	mdp4_dsi_blt_dmap_busy_wait(mfd);
	mipi_dsi_mdp_busy_wait(mfd);
	led_pwm1[1] = (unsigned char)(pwm_step);
	mipi_dsi_cmds_tx(mfd, &novatek_tx_buf, novatek_cmd_backlight_cmds,
			ARRAY_SIZE(novatek_cmd_backlight_cmds));
...
static void mipi_novatek_bkl_ctrl(struct msm_fb_data_type *mfd, bool on)
{
    ...
    if(fb_fast_backlight_valid_check == 0)
    {
       printk("!!!fast backlight on[#### unnormal] : wait 40 ms\n");
       mdelay(40);
    }
    else
    {
       mdelay(30);
       printk("!!!fast backlight on[normal] : wait 30 ms \n");
    }

	mutex_lock(&mfd->dma->ov_mutex);
    if(fb_backlight_level > 0) {
      printk("!!!fast backlight on : %d \n", fb_backlight_level);
      led_pwm1[1]  = fb_backlight_level*(PWM_MAX/mfd->panel_info.bl_max);
      mipi_dsi_cmds_tx(mfd, &novatek_tx_buf, novatek_cmd_backlight_cmds,
      ARRAY_SIZE(novatek_cmd_backlight_cmds));
    }
...
#else /* CONFIG_KTTECH_NOVATEK_BACKLIGHT */
#define DEBUG_MIPI_NOVATEK_TPS61161_BL
static void mipi_tps61161_set_backlight(struct msm_fb_data_type *mfd)
{
#ifdef DEBUG_MIPI_NOVATEK_TPS61161_BL
	pr_info("[MIPI] %s: Backlight level: %d\n", __func__, mfd->bl_level);
#endif

	if (FB_UNBLANK_FLAG) {
#ifdef DEBUG_MIPI_NOVATEK_TPS61161_BL
        pr_info("[MIPI] %s: Skip BL Level for NOVATEK HD Set Pre-Backlignt Control.\n", __func__);
#endif
		/* Skip BL Ctrl */
		return;
	}
...
static void mipi_tps61161_bkl_ctrl(struct msm_fb_data_type *mfd, bool on)
{
...
#ifndef CONFIG_KTTECH_NOVATEK_BACKLIGHT
...
static struct msm_fb_panel_data novatek_panel_data = {
	...
	.pre_bl_conrol	= mipi_novatek_bkl_ctrl,
#else
	.set_backlight	= mipi_tps61161_set_backlight, 
	.pre_bl_conrol	= mipi_tps61161_bkl_ctrl,
#endif
};
