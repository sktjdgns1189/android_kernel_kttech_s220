...
#define OVERLAY_QVGA_SIZE	0x012C00	/* 320 x 240 */
#define OVERLAY_720P_SIZE	0x0E1000	/* 1280 x 720 */
#endif
					  ...
					  struct msm_fb_data_type *mfd)
{
	...
	if (req->src.width*req->src.height <= OVERLAY_VGA_SIZE) {
		...
		min = (mfd->panel_info.yres > mfd->panel_info.xres) ?
			...
			printk(KERN_ERR "mdp4_overlay_get_perf_level max %d, min %d\n",
							 max,min);
...
#ifdef CONFIG_MACH_KTTECH
	    ((ctrl->panel_mode & MDP4_PANEL_DSI_CMD) && (req->src.width*req->src.height >= OVERLAY_720P_SIZE)) ||
#endif
...
#ifdef CONFIG_MACH_KTTECH
	/* KT Tech : TDMB LCD OFF -> ON Issue */
	if (info->node != 0 || mfd->cont_splash_done)	/* primary */
		if (!mfd->panel_power_on && req->src.format != MDP_Y_CBCR_H2V2 &&
			(req->dst_rect.w * req->dst_rect.h != OVERLAY_QVGA_SIZE))	/* suspended */
#else
	if (info->node != 0 || mfd->cont_splash_done)	/* primary */
