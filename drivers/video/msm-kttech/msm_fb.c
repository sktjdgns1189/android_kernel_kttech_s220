...
//MO2_120601 #include "takeimage.h"
#include "ollehimage.h" //MO2_120601
#endif
...
#if defined(CONFIG_FB_MSM_MIPI_DSI) && defined(CONFIG_MACH_KTTECH)
#include "mipi_dsi.h"
#endif
...
int msm_fb_get_panel_power(void)
{
	return (int)mfd_list[0]->panel_power_on;
}
EXPORT_SYMBOL(msm_fb_get_panel_power);
#endif

#ifdef CONFIG_KTTECH_BATTERY
// battery_charging 에서 fbmem.c close 함수가 호출되지 않아 LCD on/off 함수 구현
static int msm_fb_resume_sub(struct msm_fb_data_type *mfd);
static int msm_fb_suspend_sub(struct msm_fb_data_type *mfd);
int msm_fb_set_panel_power(int on)
{
	//if(!get_kttech_battch_pause()) //tempo4
	//	return 1;		
...
EXPORT_SYMBOL(msm_fb_set_panel_power);
#endif

#ifdef CONFIG_FB_MSM_LCDC_PANEL
extern void msm_fb_first_backlight(void);

#define fb_width(fb)	((fb)->var.xres)
#define fb_height(fb)	((fb)->var.yres)
#define fb_size(fb)	((fb)->var.xres * (fb)->var.yres * 2)

static void load_arbg8888_image(struct fb_info *info)
{
#if 0	//MO2_120601 Display TAKE_IMAGE
	int i;

	memset(info->screen_base, 0x0, info->fix.smem_len);

	for (i = 0; i < TAKE_IMAGE_H; i++)
	{
		memcpy ((unsigned int *)(info->screen_base + (((i + TAKE_IMAGE_Y) * (fb_width(info))) + TAKE_IMAGE_X) * 4),
			(unsigned int *)(takeimage_argb8888 + (TAKE_IMAGE_W * i * 4)),
			TAKE_IMAGE_W * 4);
	}	
#else //MO2_120601 Display OLLEH_IMAGE
	int i;
	int offset = 0;
	int image_copy_w = OLLEH_IMAGE_W * 4;
	
	memset(info->screen_base, 0x0, info->fix.smem_len);

/* olleh image(540x300) is bigger than LCD Size(400x800) */
	if ((fb_width(info)+4) < OLLEH_IMAGE_W) {
		offset = ((OLLEH_IMAGE_W - fb_width(info)) / 2) * 4;
		image_copy_w = fb_width(info) * 4;
	}
	
	for (i = 0; i < OLLEH_IMAGE_H; i++)
	{
		memcpy ((unsigned int *)(info->screen_base + (((i + OLLEH_IMAGE_Y) * (fb_width(info))) + OLLEH_IMAGE_X) * 4),
			(unsigned int *)(ollehimage_argb8888 + offset + (OLLEH_IMAGE_W * i * 4)),
			image_copy_w);
	}
#endif	//MO2_120601
}
...
static void load_arbg8888_qhd_image(struct fb_info *info)
{
	uint8 *bits, *ptr;
	int i,j,k;

	memset32_io((void *)info->screen_base, 0xFF000000, info->fix.smem_len );

	for(k=0; k<3; k++) {
		ptr = (unsigned char *)allehimage_argb8888_qhd;
		for ( i = 0 ; i < ALLEH_IMAGE_H ; i ++ ) {
			bits = (unsigned char *)info->screen_base + 
				((((i + ALLEH_IMAGE_Y) * (fb_width(info)+4)) + ALLEH_IMAGE_X) * 4);
	  
			for ( j = 0 ; j < ALLEH_IMAGE_W ; j ++ ) {
				*bits++ = *ptr++;
				dmb();
				*bits++ = *ptr++;
				dmb();
				...
				*bits++ = 0xff;
				...
				ptr++;
				dmb();
				/* For Align image width */
				if ((fb_width(info)+4) < ALLEH_IMAGE_W) {
					for(k=0; k <= ((fb_width(info)+4)-ALLEH_IMAGE_W); k++){
						bits++;
					}
				}
			}
...
void load_arbg8888_qhd_image_tsp(void)
{
	uint8 *bits, *ptr;
	int i,j,k;

	memset32_io((void *)g_info->screen_base, 0xFF000000, g_info->fix.smem_len );

	for(k=0; k<3; k++) {
		ptr = (unsigned char *)allehimage_tsp_argb8888_qhd;
		for ( i = 0 ; i < ALLEH_TSP_IMAGE_H ; i ++ ) {
			bits = (unsigned char *)g_info->screen_base + 
				((((i + ALLEH_TSP_IMAGE_Y) * (fb_width(g_info)+4)) + ALLEH_TSP_IMAGE_X) * 4);
	  
			for ( j = 0 ; j < ALLEH_TSP_IMAGE_W ; j ++ ) {
				*bits++ = *ptr++;
				dmb();
				*bits++ = *ptr++;
				dmb();
				...
				*bits++ = 0xff;
				...
				dmb();
				/* For Align image width */
				if ((fb_width(g_info)+4) < ALLEH_TSP_IMAGE_W) {
					for(k=0; k <= ((fb_width(g_info)+4)-ALLEH_TSP_IMAGE_W); k++){
						bits++;
					}
				}
			}
	...
	mdp_dma_pan_update(g_info);
}

EXPORT_SYMBOL(load_arbg8888_qhd_image_tsp);
#endif
...
static int msm_fb_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_FB_MSM_LCDC_PANEL //for fast lcd update
#if 0//def CONFIG_KTTECH_BATTERY  //tempo4
	if(!get_kttech_battch_pause())
#endif
	{
		msm_fb_open(mfd->fbi, 0);
		msm_fb_first_backlight();
	}
#endif 
	...
	msm_fb_create_sysfs(pdev);
		...
		if(get_kttech_ftm_mode()==2) {
			printk("[MDP] FTM Boot Display Mode.\n");
		} else {
			mipi_dsi_phy_ctrl(0);
			msleep(100);
			msm_fb_open(mfd->fbi, 0);
			msleep(100);
			load_arbg8888_qhd_image(mfd->fbi);
...
static int msm_fb_resume_sub(struct msm_fb_data_type *mfd)
{
	...
	printk(KERN_ERR "######## %s:  FB_BLANK_UNBLANK\n",
			__func__);
...
// MO2. YGCHOI : remove define for setting framebuffer with black buffer
// S===========================================================================
...
#ifdef CONFIG_MACH_KTTECH
int FB_UNBLANK_FLAG = 0;
EXPORT_SYMBOL(FB_UNBLANK_FLAG);
#endif
static int msm_fb_blank_sub(int blank_mode, struct fb_info *info,
			    boolean op_enable)
{
...
//daniel_del fobidden warn	static int subsys_id[2] = {MSM_SUBSYSTEM_DISPLAY,
//daniel_del fobidden warn		MSM_SUBSYSTEM_ROTATOR};
//daniel_del fobidden warn	unsigned int flags = MSM_SUBSYSTEM_MAP_IOVA;
...
#if defined(CONFIG_FB_MSM_MIPI_DSI) && defined(CONFIG_MACH_KTTECH)
	if (mfd->display_updated) {
		pdata = (struct msm_fb_panel_data *)mfd->pdev->
			dev.platform_data;
        down(&mfd->sem);
		if(pdata->pre_bl_conrol) pdata->pre_bl_conrol(mfd, 1);
		mfd->display_updated = 0;
        up(&mfd->sem);
	}
#elif defined(CONFIG_FB_MSM_LCDC) && defined(CONFIG_MACH_KTTECH)
    if (mfd->display_updated) {
      pdata = (struct msm_fb_panel_data *)mfd->pdev->
      dev.platform_data;
       if ((pdata) && (pdata->set_backlight)) {
        down(&mfd->sem);
        pdata->pre_bl_conrol(mfd, 1);
        mfd->display_updated = 0;
        up(&mfd->sem);
      }
    }
...
static int msmfb_overlay_play(struct fb_info *info, unsigned long *argp)
{
	...
	ret = mdp4_overlay_play(info, &req);
		...
		if (mfd->display_updated) {
			pdata = (struct msm_fb_panel_data *)mfd->pdev->
				dev.platform_data;
			/* Prevent MIPI-DSI LCD Refreash Issue */
			down(&mfd->sem);
			if(pdata->pre_bl_conrol) pdata->pre_bl_conrol(mfd, 1);
			mfd->display_updated = 0;
			up(&mfd->sem);
