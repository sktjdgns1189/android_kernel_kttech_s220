...
/* added by chyi, 08/21/2012 -- to solve the tdmb reset issue */
#define	VID_DEC_FREE_WAIT_FEATURE
#ifdef VID_DEC_FREE_WAIT_FEATURE
static wait_queue_head_t vid_dec_free_wait;
...
static int vid_dec_open(struct inode *inode, struct file *file)
{
	int rc = 0;
	struct video_client_ctx *client_ctx;
#ifdef VID_DEC_FREE_WAIT_FEATURE /* by chyi, 08/21/2012 -- */
	int rc2 = 0;
#endif
...
#ifdef VID_DEC_FREE_WAIT_FEATURE /* by chyi, 08/21/2012 -- */
	rc2 = wait_event_interruptible(vid_dec_free_wait, (atomic_read(&data_ready)));
	if (rc2 < 0)
		return -ENOMEM;
#endif
...
#ifndef VID_DEC_FREE_WAIT_FEATURE /* by chyi, 08/21/2012 -- */
...
static int vid_dec_release(struct inode *inode, struct file *file)
{
	...
	wake_up_interruptible(&vid_dec_free_wait);
...
#ifdef VID_DEC_FREE_WAIT_FEATURE /* by chyi, 08/21/2012 -- */
	init_waitqueue_head(&vid_dec_free_wait);
	atomic_set(&data_ready, 1);
#endif
