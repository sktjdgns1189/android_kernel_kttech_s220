...
void tps61161_set_bl_native(int lcd_backlight_level)
{
	...
	/* Set GPIO Config */
	gpio_tlmm_config(GPIO_CFG(BL_ON, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
	...
	if(lcd_backlight_level == 0) {
		...
		do {
			...
			*/
			...
			/* Set GPIO Config */
			gpio_tlmm_config(GPIO_CFG(BL_ON, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
			ret = gpio_get_value(BL_ON);
	
			if(!ret)
				klogi("[BACKLIGHT] TPS61161 Native Baclight Set Succeed.");
			else
				kloge("[BACKLIGHT] TPS61161 Native Baclight Set Failed. Re-try set Backlight.");
