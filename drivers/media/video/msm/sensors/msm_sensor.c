			...
			int update_type, int res)
{
	...
	printk("%s:  kuzuri___ : sensor_csi_setting call -> this..\n", __func__ );
	...
	int mode, int res)
{
	...
	if (s_ctrl->curr_res != res) {
		...
		s_ctrl->curr_res = res;
			...
			printk("%s:  Set to --> curr_res= %d  , res= %d \n", __func__, s_ctrl->curr_res, res);
	}
...
/* End - jaemoon.hwang@kttech.co.kr */
	...
	printk("%s: %d\n", __func__, __LINE__);
	printk(KERN_ERR "kuzuri___ : sensor_name= %s \n\n", data->sensor_name);

  printk("%s : mode= %d ,INVALID_RES= %d..\n", __func__, mode, s_ctrl->cam_mode);
	...
	if (mode != s_ctrl->cam_mode) {
		...
		if ( s_ctrl->is_csic )
		  printk("%s : sensor . is_csic = TRUE ..\n", __func__);
		if ( s_ctrl->sensordata->csi_if )
		  printk("%s : sensor . csi_if is TRUE ..\n", __func__ );

		if (s_ctrl->is_csic ||
			!s_ctrl->sensordata->csi_if)
		  {
		    printk("%s :  call sensor_csi_set()...\n", __func__ );
			rc = s_ctrl->func_tbl->sensor_csi_setting(s_ctrl,
				MSM_SENSOR_REG_INIT, 0);
		  }
		else
		  {
		    printk("%s :  call sensor_set()...\n", __func__ );
			rc = s_ctrl->func_tbl->sensor_setting(s_ctrl,
				MSM_SENSOR_REG_INIT, 0);
		  }
	}
...
int32_t msm_sensor_release(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->curr_res != MSM_SENSOR_INVALID_RES) {
	    printk("... kuzuri::: curr_res= %d.. curr_frame_length_lines= %d , pclk= %d..\n",
	    s_ctrl->curr_res, s_ctrl->curr_frame_length_lines, s_ctrl->curr_line_length_pclk );
		...
		if ( fps==0 )
		  printk("... kuzuri:: FPS is 0 ... ERROR..\n");

		//if ( fps==0 )  fps = 1 ; // kuzuri_jb test
		
		delay = 1000 / fps;
		printk("%s fps = %ld, delay = %d\n", __func__, fps, delay);
		msleep(delay);
	}
	return 0;
}
			...
			unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_SENSOR_CFG:
	  	printk("%s : cmd= SENSOR_CFG..\n", __func__ );
		return s_ctrl->func_tbl->sensor_config(s_ctrl, argp);
	case VIDIOC_MSM_SENSOR_RELEASE:
	  	printk("%s : cmd= SENSOR_RELEASE..\n", __func__ );
...
int32_t msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	cdata.cfgtype);
		...
		case CFG_SENSOR_INIT:
		  printk("___msm_sensor:  CFG_SENSOR_INIT...\n");
			if (s_ctrl->func_tbl->
			sensor_mode_init == NULL) {
				rc = -EFAULT;
				break;
			}
			printk("___msm_sensor:  CFG_SENSOR_INIT...call function\n");
			rc = s_ctrl->func_tbl->
				sensor_mode_init(
				s_ctrl,
				cdata.mode,
...
/* CONFIG_CAMERA_CALIBRATION_EEPROM */
		...
		case CFG_GET_CAL_DATA:
	  		printk("%s:  case CFG_GET_CAL_DATA:     cal_data_idx = %d\n", __func__, cdata.cfg.eeprom_value.cal_data_idx);
...
#if 1
		case CFG_SET_WB:
			printk("%s:  case CFG_SET_WB: cdata.cfg.wb_val:%d\n", __func__, cdata.cfg.wb_val);
			if (s_ctrl->func_tbl->
			sensor_set_wb == NULL) {
				rc = -EFAULT;
				break;
...
/* SENSOR_SET_BRIGHTNESS */
		...
		case CFG_SET_BRIGHTNESS:
			...
			sensor_set_brightness == NULL) {
			 ...
			 printk(" CFG_SET_BRIGHTNESS:--- func_tbl is call ...\n");
			rc = s_ctrl->func_tbl->
				sensor_set_brightness(
				s_ctrl,
...
static int cam_setup_power(int cam_type)
{
	...
	vreg_L9A_CAM_1_8 = regulator_get(NULL, str_L9A_CAM_1_8);
	...
	vreg_L8A_CAM_2_8 = regulator_get(NULL, str_L8A_CAM_2_8);
	...
	if(cam_type == 0) { // back sensor
		...
		vreg_L12A_CAM_2_8 = regulator_get(NULL, str_L12A_CAM_2_8);
...
/* End - jaemoon.hwang@kttech.co.kr */
	...
	if (data->sensor_platform_info->ext_power_ctrl != NULL)
	  {
	    printk("msm_sensor: power_up(): ext_power_ctrl is EXIST..\n");
...
request_gpio_failed:
  printk("msm_sensor_power_up():::: req_gpio_failed:....\n");
  #ifndef CONFIG_MACH_KTTECH
...
int32_t msm_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (data->sensor_platform_info->ext_power_ctrl != NULL)
	  {
	    printk("msm_sensor_power_down(): ext_power_ctrl is exist...\n");
