...
#undef CDBG
#define CDBG(fmt, args...) printk(KERN_INFO "driver.mt9e013: " fmt, ##args) 
...
// changed by hwanga 2011.03.30
uint16_t mt9e013_nl_region_boundary1 = 1;	// 3;
uint16_t mt9e013_nl_region_code_per_step1 = 25;	// 30;
uint16_t mt9e013_l_region_code_per_step = 7;	// 4;
uint16_t mt9e013_damping_threshold = 10;
uint16_t mt9e013_sw_damping_time_wait = 1;
#if 1 // #ifdef FEATURE_GET_WBVALUE 
uint16_t mt9e013_d50_rg = 768;	// default value
uint16_t mt9e013_d50_bg = 655;	// default value
#endif
...
#if 1 // #ifdef CONFIG_KTTECH_MT9E013 // jykim 110322
static bool PREV_1080P_SUPPORT; 
#endif

static bool CSI_CONFIG;
static struct mt9e013_ctrl_t *mt9e013_ctrl = NULL;
...
static int32_t mt9e013_set_fps(struct fps_cfg   *fps)
{
	...
	//mt9e013_group_hold_on(); // preview 중에 fps 조정하면 driver 멈춤 
...
#if 1 // #ifdef CONFIG_KTTECH_MT9E013 
	if (mt9e013_ctrl->sensormode == SENSOR_PREVIEW_MODE) {
		// mt9e013_group_hold_on();
		if (mt9e013_ctrl->my_reg_gain != gain) {
			mt9e013_ctrl->my_reg_gain = gain;
			gain |= 0x1000;
			// CDBG("%s: gain(%d) my_reg_gain(%d)\n", __func__, gain, mt9e013_ctrl->my_reg_gain);

			rc = mt9e013_i2c_write_w_sensor(REG_GLOBAL_GAIN, gain);
		}

		if (mt9e013_ctrl->my_reg_line_count != (uint16_t) line) {
			mt9e013_ctrl->my_reg_line_count = (uint16_t) line;
			line = (uint32_t) (line * mt9e013_ctrl->fps_divider /
						   0x00000400);  // preview보다 capture가 밝음 버그 수정 
			// CDBG("%s: line(%d) my_reg_line_count(%d)\n", __func__, line, mt9e013_ctrl->my_reg_line_count);
			rc = mt9e013_i2c_write_w_sensor(REG_COARSE_INTEGRATION_TIME, line);
		}
		// mt9e013_group_hold_off();
	} else {
		mt9e013_ctrl->my_reg_line_count = (uint16_t) line;
		line = (uint32_t) (line * 2 * mt9e013_ctrl->pict_fps_divider / 0x00000400);
...
#if 1 // #ifdef CONFIG_KTTECH_MT9E013 // Added by hwanga for loading Calibration Data - 2011.03.30
static int32_t mt9e013_set_lc(void)
{
	unsigned short calData, lscAddr;
	int32_t rc, i, j;
	int16_t lscOffset = 0x3802;

	// Initialize OTPM for reading
	// mt9e013_i2c_write_w_sensor(0x301A, 0x067c); // RESET_REGISTER_REG_RD_EN
	// mt9e013_i2c_write_w_sensor(0x301A, 0x0610);	// disable streaming
	mt9e013_i2c_write_w_sensor(0x3134, 0xCD95);	// timing parameters for otp read
	mt9e013_i2c_write_w_sensor(0x304C, 0x3200); // choose to only read record type 0x32
	mt9e013_i2c_write_w_sensor(0x304A, 0x0010); // auto read start
	
	//POLL 0x304A bits 5 and 6 for read end and success
	i = 0;
#if 1 // CTS test=> #if 0 => testPreviewSizesCombinations is passed
	do 	{
		msleep(20);
		mt9e013_i2c_read(0x304A, &calData, 2);
		//CDBG("%s: read command = 0x%x(%d)\n", __func__, calData, i);
		i++;
	} while ((calData != 0x0060) && i < 3); // Error Fix

	rc = mt9e013_i2c_read(0x3800, &calData, 2);
	//CDBG("%s: calData = 0x%x\n", __func__, calData);
#endif
	if (calData == lscOffset)  // TBD : it should be the first LSC data address
		{
		for (i = 0; i < 5; i++)
		{
			for (j = 0; j < 20; j++)
			{
				rc = mt9e013_i2c_read(lscOffset + 40*i + 2*j, &calData, 2);
				lscAddr = 0x3600 + 64*i + 2*j;
				mt9e013_i2c_write_w_sensor(lscAddr, calData);
			}
		}
	
		rc = mt9e013_i2c_read(lscOffset + 100*2, &calData, 2);
		mt9e013_i2c_write_w_sensor(0x3782, calData);
		
		rc = mt9e013_i2c_read(lscOffset + 101*2, &calData, 2);
		mt9e013_i2c_write_w_sensor(0x3784, calData);
	
		// for MT9E013 only
		for (i = 0; i < 4; i++)
		{
			rc = mt9e013_i2c_read(lscOffset + 102*2 + 2*i, &calData, 2);
			mt9e013_i2c_write_w_sensor(0x37C0 + 2*i, calData);
		}
		// for MT9E013 only - end
		
		// for calibrated rg, bg data for D50
		mt9e013_i2c_read(0x38D6, &calData, 2);	// rg
		CDBG("%s: ======before=======  awb cal : rg= %d, calData= %d\n", __func__, mt9e013_d50_rg, calData);

		if ((calData > mt9e013_d50_rg - 120) && (calData < mt9e013_d50_rg + 120))	// +-10%
			mt9e013_d50_rg = calData;
		else
			CDBG("%s: awb cal rg range-over!(%d)\n", __func__, calData);

		mt9e013_i2c_read(0x38D8, &calData, 2);	// bg
		CDBG("%s: =====before========  awb cal : bg= %d, calData= %d\n", __func__, mt9e013_d50_bg, calData);
		if ((calData > mt9e013_d50_bg - 100) && (calData < mt9e013_d50_bg + 100))
			mt9e013_d50_bg = calData;
		else
			CDBG("%s: awb cal bg range-over!(%d)\n", __func__, calData);
		
		CDBG("%s: awb cal : rg= %d, bg= %d\n", __func__, mt9e013_d50_rg, mt9e013_d50_bg);
		...
		{
		rc = mt9e013_i2c_write_w_table(mt9e013_regs.reg_lsc,
				mt9e013_regs.reg_lsc_size);
		if (rc < 0)
			return rc;
		}
...
static int32_t mt9e013_sensor_setting(int update_type, int rt)
{
	...
	if (update_type == REG_INIT) {
		...
		mt9e013_set_lc();	// added by hwanga 2011.03.30
		cam_debug_init();
		CSI_CONFIG = 0;
	} else if (update_type == UPDATE_PERIODIC) {
			msleep(100);
		if (rt == RES_PREVIEW) {
#if 0 // back to reg_rec with new mipi timing
			// MIPI Timing - causing zipper issue
			mt9e013_i2c_write_w_sensor(0x31B4, 0x0E77); 	// MIPI_TIMING_0
			mt9e013_i2c_write_w_sensor(0x31B6, 0x0D24); 	// MIPI_TIMING_1
			mt9e013_i2c_write_w_sensor(0x31B8, 0x020E); 	// MIPI_TIMING_2
			mt9e013_i2c_write_w_sensor(0x31BA, 0x0710); 	// MIPI_TIMING_3
			mt9e013_i2c_write_w_sensor(0x31BC, 0x2A0D); 	// MIPI_TIMING_4
#endif
			mt9e013_i2c_write_w_table(mt9e013_regs.reg_prev,
				mt9e013_regs.reg_prev_size);
			mt9e013_ctrl->my_reg_line_count 
				= mt9e013_regs.reg_prev[E013_COARSE_INTEGRATION_TIME].wdata;
		} else {
			mt9e013_i2c_write_w_table(mt9e013_regs.reg_snap,
				mt9e013_regs.reg_snap_size);
			mt9e013_ctrl->my_reg_line_count 
				= mt9e013_regs.reg_snap[E013_COARSE_INTEGRATION_TIME].wdata;

		}
		 
#if 1 // #ifdef CONFIG_KTTECH_MT9E013 // jykim 110322
				if(PREV_1080P_SUPPORT) {
					// [1080p - 80%FOV]
					// Output Size
					CDBG("%s: PREV_1080P_SUPPORT \n", __func__);
		
					// MIPI Timing - causing zipper issue
					mt9e013_i2c_write_w_sensor(0x31B4, 0x0E99); 	// MIPI_TIMING_0
					mt9e013_i2c_write_w_sensor(0x31B6, 0x0E13); 	// MIPI_TIMING_1
					mt9e013_i2c_write_w_sensor(0x31B8, 0x0E3F); 	// MIPI_TIMING_2
					mt9e013_i2c_write_w_sensor(0x31BA, 0x0710); 	// MIPI_TIMING_3
					mt9e013_i2c_write_w_sensor(0x31BC, 0x000D); 	// MIPI_TIMING_4
		
					mt9e013_i2c_write_b_sensor(0x0104, 0x01);	// {0x0104, 0x01},			  // GROUPED_PARAMETER_HOLD
					mt9e013_i2c_write_w_sensor(0x0344, 0x0138); // {0x0344, 0x0138},		  // X_ADDR_START
					mt9e013_i2c_write_w_sensor(0x0348, 0x0B97); // {0x0348, 0x0B97},		  // X_ADDR_END
					mt9e013_i2c_write_w_sensor(0x0346, 0x01E0); // {0x0346, 0x01E0},		  // Y_ADDR_START
					mt9e013_i2c_write_w_sensor(0x034A, 0x07BD); // {0x034A, 0x07BD},		  // Y_ADDR_END
					mt9e013_i2c_write_w_sensor(0x034C, 0x0790); // {0x034C, 0x0790},		  // X_OUTPUT_SIZE
					mt9e013_i2c_write_w_sensor(0x034E, 0x0448); // {0x034E, 0x0448},		  // Y_OUTPUT_SIZE
					
					mt9e013_i2c_write_w_sensor(0x306E, 0xFC80); // {0x306E, 0xFC80},		  // DATAPATH_SELECT
					mt9e013_i2c_write_w_sensor(0x3040, 0x0041); // {0x3040, 0x0041},		  // READ_MODE*/
					mt9e013_i2c_write_w_sensor(0x30B0, 0x0000); // {0x30B0, 0x0000},		  // 2nd Shutter Disable
					mt9e013_i2c_write_w_sensor(0x3178, 0x0000); // {0x3178, 0x0000},		  // ANALOG_CONTROL5
					mt9e013_i2c_write_w_sensor(0x3ED0, 0x1E24); // {0x3ED0, 0x1E24},		  // DAC_LD_4_5
					mt9e013_i2c_write_w_sensor(0x0400, 0x0002); // {0x0400, 0x0002},		  // SCALING_MODE
					mt9e013_i2c_write_w_sensor(0x0404, 0x0016); // {0x0404, 0x0016},		  // SCALE_M
					mt9e013_i2c_write_w_sensor(0x31E0, 0x00B1); // {0x31E0, 0x00B1},		  // PIX_DEF_ID
					mt9e013_i2c_write_w_sensor(0x31E6, 0x07FC); // {0x31E6, 0x07FC},		  // PIX_DEF_ID_2
		
					// Timing
					mt9e013_i2c_write_w_sensor(0x0342, 0x100C); // {0x0342, 0x100C},		  // LINE_LENGTH_PCK
					mt9e013_i2c_write_w_sensor(0x0340, 0x066D); // {0x0340, 0x066D},		  // FRAME_LENGTH_LINES
					mt9e013_i2c_write_w_sensor(0x3014, 0x03F6); // {0x3014, 0x03F6},		  // FINE_INTEGRATION_TIME
					mt9e013_i2c_write_w_sensor(0x3010, 0x0078); // {0x3010, 0x0078},		  // FINE_CORRECTION
					mt9e013_i2c_write_b_sensor(0x0104, 0x00);	// {0x0104, 0x00},			  // GROUPED_PARAMETER_HOLD
			}
#endif

		if (!CSI_CONFIG) {
#if 1 // Mo2_hyeonwoo GB unabled // #ifndef CONFIG_KTTECH_MT9E013 // jykim : undefined symbol
	...
	int res)
{
	...
	CDBG("%s: o3ics_1080 mode = %d  \n", __func__, mode);
	switch (mode) {
	case SENSOR_PREVIEW_MODE:
#if 1 // #ifdef CONFIG_KTTECH_MT9E013 // jykim 110322
				if(res == SENSOR_1080P_SIZE) {
						PREV_1080P_SUPPORT = 1;
				} 
				else {
						PREV_1080P_SUPPORT = 0;
				}
				CDBG("%s: PREV_1080P_SUPPORT=%d \n", __func__, PREV_1080P_SUPPORT);
#endif
...
{
#if 1 // #ifdef CONFIG_KTTECH_MT9E013 // jykim
	CDBG("%s: e\n", __func__);

	if (mt9e013_ctrl->sensordata->cam_shutdown != NULL)
	  mt9e013_ctrl->sensordata->cam_shutdown(&mt9e013_client->dev, 0); // Back Camera
...
#if 1 // #ifdef CONFIG_KTTECH_MT9E013 // jykim
	CDBG("%s: %d  cam_setup\n", __func__, __LINE__);
	data->cam_setup(&mt9e013_client->dev, 0); // Back Camera
	...
	const struct i2c_device_id *id)
{
	...
	mt9e013_client->addr = mt9e013_client->addr ; //0503 o3ics_1080
...
#ifdef FEATURE_GET_WBVALUE  //boba
		case CFG_GET_WB_VALUE:	
			  cdata.cfg.wb_value.rvalue = mt9e013_d50_rg;
			  cdata.cfg.wb_value.bvalue = mt9e013_d50_bg;
...
static int mt9e013_sensor_release(void)
{
	...
	mutex_lock(&mt9e013_mut);
	mt9e013_power_down();
#if 1 // Mo2_hyeonwoo GB unabled // #ifndef CONFIG_KTTECH_MT9E013 // jykim remove warning
		...
		struct msm_sensor_ctrl *s)
{
	...
	s->s_mount_angle = info->sensor_platform_info->mount_angle; // Mo2_hyeonwoo GB set 0;
#endif
